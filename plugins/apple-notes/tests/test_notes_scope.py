...
class TestCanonicalScope(unittest.TestCase):
    ...
    def test_list_folders_filters_canonical(self):
        ...
        with with_scope("iCloud/Work"):
            with patch.object(
                notes, "_run_jxa", return_value=json.dumps(all_paths)
            ):
    ...
    def test_create_note_rejects_out_of_scope_full_path(self):
        with with_scope("iCloud/Work"):
            with patch.object(
                notes, "_run_jxa", side_effect=AssertionError("must not reach JXA")
            ):
                with self.assertRaises(notes.NotesError):
    ...
    def test_require_note_in_scope_full_path(self):
        with with_scope("iCloud/Work"):
            with patch.object(
                notes, "_run_jxa", return_value=json.dumps("iCloud/Work")
            ):
                self.assertEqual(
                    notes._require_note_in_scope("some-id"), "iCloud/Work"
                )
    ...
    def test_get_note_enforces_full_path(self):
        ...
        with with_scope("iCloud/Work"):
            with patch.object(
                notes, "_run_jxa", return_value=json.dumps(note)
            ):
                with self.assertRaises(notes.NotesError):
                    notes.get_note("1", format="plaintext")
        with with_scope("On My Mac/Work"):
            with patch.object(
                notes, "_run_jxa", return_value=json.dumps(dict(note))
            ):
    ...
    def test_main_tools_return_call_tool_result(self):
        ...
        tool_funcs = [n for n in funcdefs if any(
            isinstance(d, ast.Call)
            and getattr(getattr(d, "func", None), "attr", "") == "tool"
            for d in n.decorator_list
        )]
        self.assertGreaterEqual(len(tool_funcs), 11)
        for fn in tool_funcs:
            self.assertIsNotNone(
                fn.returns, f"tool {fn.name} must have a return annotation"
            )
            self.assertEqual(
