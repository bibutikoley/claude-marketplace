...
Or standalone, without the marketplace (no clone needed), pinned to `v0.4.0`
(recommended — reproducible; drop `@v0.4.0` to track `main`):
...
- Claude Code CLI: just run `claude mcp add apple-notes-mcp -s user -- uvx --from "git+https://github.com/bibutikoley/claude-marketplace@v0.4.0#subdirectory=plugins/apple-notes-mcp" apple-notes-mcp`, then skip to Step 6.
...
- opencode form (merge under top-level "mcp"):
  {"mcp": {"apple-notes-mcp": {"type": "local", "command": ["uvx", "--from", "git+https://github.com/bibutikoley/claude-marketplace@v0.4.0#subdirectory=plugins/apple-notes-mcp", "apple-notes-mcp"]}}}

If I say I have a local clone of bibutikoley/claude-marketplace, use it instead: replace the --from value with the absolute path to its plugins/apple-notes-mcp directory (absolute path only, never relative).
...
# e.g. /Users/you/claude-marketplace/plugins/apple-notes-mcp
```
...
- All snippets above default to the pinned `@v0.4.0` form. To track `main`
  instead (mutable, not reproducible), drop the `@v0.4.0` from the URL.
